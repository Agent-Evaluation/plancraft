...
from .nvidia_llm import call_nvidia_with_retry, DEFAULT_MODEL
import re
...
class NvidiaSingleAgent:
    """
    Single Agent using the NVIDIA NIM API (Kimi K2.5).
    Standalone class — does not depend on google.genai.
    """
    ...
    def act(self, observation_text: str) -> str:
        ...
        # 2. Call NVIDIA API
        action_text = call_nvidia_with_retry(
            self.model_name,
            self.conversation,
            SYSTEM_PROMPT,
        )
